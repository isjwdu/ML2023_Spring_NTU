...
Original file is located at
    https://colab.research.google.com/drive/1OChZFBcRwJ08rs6Q_Zg-PLnYi-6k5w3j
...
class Classifier(nn.Module):
    def __init__(self, d_model=512, n_spks=600, dropout=0.0):
        super().__init__()
        ...
        self.SelfAttentionPooling = SelfAttentionPooling(d_model)
    ...
    def forward(self, mels):
        ...
        out = self.prenet(mels) # (batch size, length, 40)
        # out: (length, batch size, d_model)
        out = out.permute(1, 0, 2) # (length, batch size, 40)
        # The encoder layer expect features in the shape of (length, batch size, d_model).
        # out = self.encoder_layer(out) # Trnasfomer可以隱藏的傳length因為已經是那個形狀了
        out = self.encoder_layer(out)
        #out = self.encoder_layer(out, lengths=lengths)
        # out: (batch size, length, d_model)
